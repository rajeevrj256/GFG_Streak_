...
class Solution{
public:
    void tre(node* root,vector<int>&vec,int index){
        ...
        node * left= newNode(vec[leftidx]);
        root->left=left;
        tre(root->left,vec,index*2+1);
        
         }
         
         if(rightidx<vec.size()){
             
        node* right=newNode(vec[index*2+2]);
        root->right=right;
        tre(root->right,vec,index*2+2);
         }
